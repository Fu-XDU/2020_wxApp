...
Page({
  ...
  submit_form: function() {
    ...
    util.getHttpTime("yyyy-MM-dd HH:mm:ss").then((res) => {
      ...
      this.getOrderNumber().then((res) => {
        util.httpsGet("/api/db?sql=INSERT INTO Exp_Orders(name, phone, dormitory, dormitoryid, delivery_station, delivery_code, size, weight, remarks, state, order_time, order_number, openid,money)VALUES(\"" + this.data.name + '","' + this.data.phone + '","' + this.data.dormitory + '","' +
          this.data.dormitoryid + '","' + this.data.delivery_station + '","' + this.data.delivery_code + '","' + this.data.size + '","' + this.data.weight + '","' + this.data.remarks + '","' + this.data.state + '","' + this.data.order_time + '","' + this.data.order_number + '","' + app.globalData.openid + '","' +this.data.money + '")').then((res) => {
          if (res.data == '1') {
  ...
  bindDormitoriesPickerChange: function(e) {
    ...
    this.setData({
      ...
      dormitory: _this.data.dormitories_list[e.detail.value]
    })
  },
  //处理大小选择器事件
  bindSizePickerChange: function(e) {
    ...
    this.setData({
      ...
      size: _this.data.size_list[e.detail.value],
      money: _this.data.money_list[e.detail.value]
    })
  },
  //处理重量选择器事件
  bindWeightPickerChange: function(e) {
    ...
    _this.setData({
      ...
      weight: _this.data.weight_list[e.detail.value]
    })
  },
  /**
   * 生命周期函数--监听页面隐藏
